...
from component_interfaces import (
    IContextSynthesizer, ReasoningSession, ReasoningRound, ReasoningConfig
)
...
class ContextSynthesizer(IContextSynthesizer):
    ...
    def _summarize_tool_usage(self, tool_history: List[Dict]) -> str:
        ...
        for tool_use in tool_history[-6:]:  # Last 6 tool uses
            ...
            if tool_name == "search_course_content":
                ...
                search_attempts.append(f"{status} searched '{query}' {f'in {course}' if course else ''}")
    ...
    def _analyze_tool_patterns(self, tool_executions: List) -> List[str]:
        ...
        return [tool_exec.tool_name for tool_exec in tool_executions if tool_exec.success]
    ...
    def _extract_key_terms(self, query: str) -> str:
        ...
        stopwords = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'what', 'how', 'when', 'where', 'why'}
