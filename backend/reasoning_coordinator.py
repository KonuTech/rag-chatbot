...
class ReasoningCoordinator(IReasoningCoordinator):
        ...
        try:
            ...
            while session.termination_reason is None:
                ...
                session.total_tokens += (
                    round_result.token_usage.get("total", 0)
                    if round_result.token_usage
                    else 0
                )
    ...
    async def _execute_reasoning_round(
        ...
        try:
            ...
            context_briefing = ""
            if round_number > 0:
                context_briefing = self.context_synthesizer.build_context_briefing(
    ...
    def _complete_session(self, session: ReasoningSession) -> None:
        ...
        elif session.termination_reason in [
    ...
    def _update_session_context(
        self, session: ReasoningSession, round_result: ReasoningRound
    ) -> None:
        ...
        if round_result.final_text:
            session.reasoning_trace.append(
                f"Round {round_result.round_number}: {round_result.final_text[:100]}..."
            )

        # Update evolving intent
        session.evolving_intent = self.context_synthesizer.update_intent_understanding(
