...
from component_interfaces import (
    IReasoningEngine, ReasoningRound, ReasoningConfig,
    APIError
)
...
"""
    ...
    async def execute_reasoning_round(
        ...
        try:
            ...
            # Build user message
            user_message = self._build_user_message(query, round_number, context_briefing)
    ...
    def _build_user_message(self, query: str, round_number: int, context_briefing: str) -> str:
    ...
    async def _make_api_call_with_retries(self, api_params: Dict[str, Any]):
        ...
        # All retries failed
        raise APIError(f"API call failed after {self.max_retries} attempts: {str(last_exception)}") from last_exception
    
    def _calculate_token_usage(self, api_params: Dict[str, Any], response) -> Dict[str, int]:
        """
        ...
        for message in api_params.get("messages", []):
            message_tokens += len(str(message.get("content", ""))) * self.avg_tokens_per_char
        ...
        if response.content:
            for content_block in response.content:
                if hasattr(content_block, 'text'):
                    ...
                    output_tokens += len(str(content_block.input)) * self.avg_tokens_per_char
