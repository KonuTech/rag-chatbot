...
from component_interfaces import (
    IResponseAssembler, ReasoningSession, TerminationReason, ReasoningConfig
)
...
class ResponseAssembler(IResponseAssembler):
    ...
    def handle_partial_completion(
        ...
        if termination_reason == TerminationReason.MAX_ROUNDS_REACHED:
            ...
            if best_response:
                ...
                response = "The query is quite complex. Based on the information I found: " + self._synthesize_findings(session)
                return response, sources
            ...
            if best_response:
                ...
                response = "I encountered some technical difficulties while searching, but here's what I can tell you: " + self._synthesize_findings(session)
                return response, sources
    ...
    def extract_sources(self, session: ReasoningSession) -> List[Dict[str, Any]]:
        ...
        for round_data in session.rounds:
            for tool_exec in round_data.tool_executions:
                if tool_exec.success and tool_exec.tool_name.startswith("search_course_content"):
                    # Extract sources from search results
                    round_sources = self._extract_sources_from_search_result(tool_exec.result)
    ...
    def _handle_natural_completion(self, session: ReasoningSession) -> Tuple[str, List[Dict[str, Any]]]:
        ...
        if final_round.final_text:
            ...
            response = self._find_best_response(session) or "I've processed your question but couldn't generate a final response."
        
        sources = self.extract_sources(session)
        return response, sources
    
    def _handle_no_rounds(self, termination_reason: TerminationReason) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    def _extract_meaningful_content(self, tool_result: str, max_length: int = 200) -> str:
        """Extract meaningful content from tool results"""
        if not tool_result or tool_result.startswith("No") or tool_result.startswith("Error"):
            return ""
    ...
    def _extract_sources_from_search_result(self, search_result: str) -> List[Dict[str, Any]]:
        """
