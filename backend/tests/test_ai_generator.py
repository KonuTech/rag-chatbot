...
class TestAIGenerator:
    ...
    def setup_method(self):
        ...
        self.ai_generator = AIGenerator(api_key="test-key", model="claude-sonnet-4-20250514")
    ...
    def test_generate_response_with_tools_no_tool_use(self):
        ...
        mock_response = Mock()
        mock_response.content = [Mock(text="This is a general answer without using tools")]
        mock_response.stop_reason = "end_turn"
        self.mock_client.messages.create.return_value = mock_response
    ...
    def test_generate_response_with_tool_use(self):
        ...
        mock_final_response.content = [Mock(text="Based on the course content, Python is a programming language...")]
        ...
        # Mock tool manager
        mock_tool_manager = Mock()
        mock_tool_manager.execute_tool.return_value = "Course content about Python basics from Lesson 1"
    ...
    def test_handle_tool_execution_with_multiple_tools(self):
        ...
        mock_tool_manager.execute_tool.assert_any_call("get_course_outline", course_name="Python Fundamentals")
    ...
    def test_tool_execution_error_handling(self):
        ...
        mock_tool_manager = Mock()
        mock_tool_manager.execute_tool.return_value = "Tool error: Database connection failed"
...
class TestAIGeneratorV2Compatibility:
    ...
    def test_v2_multi_round_capability(self, mock_asyncio_run):
        ...
        self.ai_generator_v2.response_assembler.assemble_final_response.return_value = (
            "Multi-round response with sources", [{"text": "Source 1", "url": None}]
        )
    ...
    async def test_v2_async_interface(self):
        ...
        with patch.object(self.ai_generator_v2.reasoning_coordinator, 'process_query') as mock_process:
            mock_session = Mock()
            mock_process.return_value = mock_session
            
            # Configure response assembler
            with patch.object(self.ai_generator_v2.response_assembler, 'assemble_final_response') as mock_assemble:
...
class TestMigrationHelper:
    ...
    def test_interface_compatibility(self):
        ...
        v1_methods = {method for method in dir(AIGenerator) if not method.startswith('_')}
        
        if HAS_V2:
            v2_methods = {method for method in dir(AIGeneratorV2) if not method.startswith('_')}
