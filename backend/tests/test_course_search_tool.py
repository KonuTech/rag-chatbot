...
class TestCourseSearchTool:
    ...
    def test_execute_with_successful_search_results(self):
        ...
        mock_results = SearchResults(
            documents=[
                "This is lesson content about Python basics",
                "Another lesson about variables",
            ],
            metadata=[
        ...
        assert (
            self.search_tool.last_sources[0]["text"] == "Python Fundamentals - Lesson 1"
        )
        assert (
            self.search_tool.last_sources[1]["text"] == "Python Fundamentals - Lesson 2"
        )
    ...
    def test_execute_with_both_filters(self):
        ...
        result = self.search_tool.execute(
            "pandas", course_name="Data Science", lesson_number=2
        )
    ...
    def test_execute_with_empty_results_and_filters(self):
        ...
        result = self.search_tool.execute(
            "nonexistent", course_name="Unknown Course", lesson_number=99
        )

        assert (
            "No relevant content found in course 'Unknown Course' in lesson 99"
            in result
        )
