...
class TestReasoningEngine:
    ...
    def mock_anthropic_client(self):
        ...
        mock_response_final.content = [
            Mock(text="Here's information about Python programming...")
        ]
    ...
    @pytest.mark.asyncio
    async def test_execute_reasoning_round_with_tools(
        self, reasoning_engine, mock_anthropic_client
    ):
        ...
        result = await reasoning_engine.execute_reasoning_round(
            query="What is Python?", context_briefing="", tools=tools, round_number=0
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_reasoning_round_without_tools(
        self, reasoning_engine, mock_anthropic_client
    ):
    ...
    def test_token_usage_estimation(self, reasoning_engine):
        ...
        tokens = reasoning_engine.estimate_token_usage(
            context_briefing="Some previous context", query="What is machine learning?"
        )
...
class TestContextSynthesizer:
    ...
    def sample_session(self):
        ...
        session = ReasoningSession(
            ...
            reasoning_trace=[
                "Round 0: Searched for Python info",
                "Round 1: Searched for Java info",
            ],
    ...
    def test_build_context_briefing_second_round(
        self, context_synthesizer, sample_session
    ):
...
class TestToolDispatcher:
    ...
    @pytest.mark.asyncio
    async def test_execute_tools_with_fallback(
        self, tool_dispatcher, mock_tool_manager
    ):
    ...
    def test_can_retry_tool(self, tool_dispatcher):
        ...
        assert not tool_dispatcher.can_retry_tool(
            "search", Exception("Invalid parameter")
        )
...
class TestResponseAssembler:
    ...
    def test_assemble_final_response_natural_completion(
        self, response_assembler, completed_session
    ):
        """Test response assembly for naturally completed session"""
        response, sources = response_assembler.assemble_final_response(
            completed_session
        )

        assert "Python is a high-level programming language" in response
        assert (
            len(sources) > 0 if sources else True
        )  # May or may not have sources depending on extraction

    def test_handle_partial_completion_max_rounds(
        self, response_assembler, completed_session
    ):
...
class TestReasoningCoordinator:
    ...
    def coordinator(self, mock_components):
        ...
        reasoning_engine, context_synthesizer, tool_dispatcher, response_assembler = (
            mock_components
        )
        config = ReasoningConfig()
    ...
    @pytest.mark.asyncio
    async def test_process_query_single_round_completion(
        self, coordinator, mock_components
    ):
    ...
    async def test_process_query_max_rounds_reached(self, coordinator, mock_components):
        ...
        session = await coordinator.process_query(
            "Complex query requiring multiple rounds"
        )
...
class TestAIGeneratorV2Integration:
    ...
    def test_generate_response_with_tools(
        self, mock_asyncio_run, ai_generator_v2, mock_tool_manager
    ):
...
class TestEndToEndScenarios:
    ...
    async def test_multi_course_comparison_scenario(self):
        ...
        tool_dispatcher.execute_tools.return_value = [
            ToolExecutionResult(
                "search_course_content", {"query": "Python"}, True, "Python info", 1.0
            ),
            ToolExecutionResult(
                "search_course_content", {"query": "Java"}, True, "Java info", 1.0
            ),
        ]
...
class TestPerformanceAndLimits:
    ...
    def test_token_limit_enforcement(self):
        ...
        estimated_tokens = reasoning_engine.estimate_token_usage(
            large_context, large_query
        )
